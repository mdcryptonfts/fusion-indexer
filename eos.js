...
const RPC_ENDPOINTS = {
    ...
    'wax': 'https://api.waxdaobp.io',
    'telos': 'https://telos.greymass.com',
}

async function getEosBalance(account_name, chain = "eos") {
    return axios.post(`${RPC_ENDPOINTS[chain]}/v1/chain/get_account`, { account_name, })
}
...
async function get_staked(account_name, symbol, chain = "eos") {
    ...
    try {
        let refunding = 0;
        if(response.data.refund_request){
            refunding += parseFloat(response.data.refund_request.cpu_amount);
            refunding += parseFloat(response.data.refund_request.net_amount);
        }
        return refunding + (response.data.voter_info.staked / (10 ** get_precision(symbol)))
    } catch (e) {
        return 0;
    }
}
